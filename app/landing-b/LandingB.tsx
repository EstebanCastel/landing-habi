...
export default function LandingB({ properties, dealUuid }: LandingBProps) {
  ...
  return (
    <div key={`${dealUuid}-${_forceUpdate}`} className="min-h-screen bg-gray-50">
      ...
      {/* Banner Contacta a tu Ejecutivo - Solo MX */}
      {isMX && properties.whatsapp_asesor && (
        <div className="max-w-xl mx-auto px-4 sm:px-6 mt-4 sm:mt-6">
          <button
            onClick={() => handleWhatsAppRedirect('propuesta')}
            className="w-full rounded-xl sm:rounded-2xl shadow-md overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-[1.02] cursor-pointer border-0 p-0"
            style={{ background: "linear-gradient(90deg, #7400C2 0%, #430070 100%)" }}
          >
            <div className="flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-4 sm:py-5">
              ...
              <div className="flex-1 text-left">
                <p className="text-white font-bold text-sm sm:text-base">
                  ¡No te quedes sin tu oferta!
                </p>
                <p className="text-white/80 text-xs sm:text-sm">
                  Acepta ahora dando clic aquí y habla con tu ejecutivo
                </p>
              </div>
              <div className="flex-shrink-0">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
