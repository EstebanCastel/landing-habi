...
export default function NegotiationSystem({ currentPrice, dealUuid, enabled, precioIntermedio, precioMaximo, whatsappAsesor, onPriceNegotiated, costBreakdownRead }: NegotiationSystemProps) {
    ...
    const loadSession = async () => {
      try {
        ...
        if (data.session?.messages?.length > 0) {
          const msgs: BidEntry[] = data.session.messages;
          setHistory(msgs);
          setLastHabiOffer(data.session.last_habi_offer || currentPrice);
          setAgreed(data.session.agreed || false);
          setAgreedPrice(data.session.agreed_price || 0);
        ...
        console.warn('[Negotiation] Failed to load session:', e);
      }
    };
    loadSession();
  ...
  useEffect(() => {
    ...
    fetch('/api/negotiation', {
      ...
      body: JSON.stringify({
        ...
        last_habi_offer: lastHabiOffer,
        agreed,
        agreed_price: agreedPrice,
      }),
    })
      .catch(e => console.warn('[Negotiation] Failed to save session:', e))
      .finally(() => { savingRef.current = false; });
  }, [history, agreed, agreedPrice, lastHabiOffer, enabled, dealUuid]);
  ...
  const handleSubmitBid = () => {
    ...
    if (lastHabiOffer < precioMaximo - 1000000) {
      ...
      setFinalChance(true);
      addHabiResponse([{ from: 'habi', amount: offerPreFinal, message: `Hemos hecho un esfuerzo adicional. Podemos ofrecerte ${formatPrice(offerPreFinal)}.` }]);
      return;
    }

    addHabiResponse([{ from: 'habi', amount: lastHabiOffer, message: `Nuestra oferta de ${formatPrice(lastHabiOffer)} es la mejor que podemos hacer.` }]);
  };
  ...
  const handleReject = () => {
    ...
    if (finalChance && lastHabiOffer < precioMaximo) {
      ...
      addHabiResponse([{ from: 'habi', amount: finalOffer, message: `Espera, hemos hecho un último esfuerzo. Te ofrecemos ${formatPrice(finalOffer)}. Es nuestro máximo absoluto.` }]);
      return;
    }
  ...
  const handleReviewOffer = () => {
    if (onPriceNegotiated && agreedPrice > 0) onPriceNegotiated(agreedPrice);
    setIsOpen(false);
  };
  ...
  return (
    <>
      ...
      {hasBeenTriggered && !isOpen && (
        <div className="fixed bottom-16 right-6 sm:bottom-6 sm:right-auto sm:left-6 z-[99]">
          <button
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-end sm:items-center justify-center">
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col max-h-[85vh] relative overflow-hidden">
            ...
            {showConfirm && (
              <div className="absolute inset-0 z-20 bg-white sm:rounded-2xl rounded-t-2xl flex flex-col items-center justify-center p-6">
                ...
                <button
                  ...
                  className="w-full bg-gray-100 text-gray-700 py-3 rounded-xl font-semibold mb-2 hover:bg-gray-200 transition text-sm"
                >
                  Sí, cerrar
                </button>
                {lastHabiOffer >= precioMaximo ? (
            ...
            <div className="bg-gradient-to-r from-purple-600 to-purple-800 p-4 sm:rounded-t-2xl rounded-t-2xl flex justify-between items-center">
              ...
              <button onClick={() => { if (agreed) { if (onPriceNegotiated && agreedPrice > 0) onPriceNegotiated(agreedPrice); setIsOpen(false); } else { setShowConfirm(true); } }} className="text-purple-200 hover:text-white p-1">
                  ...
                  <div className="text-center">
                    ...
                    {cededAmount > 0 && (
                      <p className="text-[10px] text-gray-400 mt-1">
                        +{formatPrice(cededAmount)} ({cededPct}%) sobre la oferta inicial
                      </p>
                    )}
                  </div>
